...
function AppProvider({ children }) {
  ...
  const refreshAuctions = React.useCallback(
    async (addr = walletAddress) => {
      try {
        const res = await fetch("http://34.136.22.54:9092/contracts");
...
await fetch("http://34.136.22.54:9092/contracts", {
  method: "POST",
...
await refreshAuctions();
  ...
  async function deployContractsCustom() {
    ...
    try {
      ...
      const lvContract = await deployWithGas(LendingVaultFactory, [purchaseToken]);
      await lvContract.deployed();
      setLendingVaultAddress(lvContract.address);
...
alert("All contracts deployed successfully!");
  ...
  useEffect(() => {
    fetch("http://34.136.22.54:9092/contracts")
